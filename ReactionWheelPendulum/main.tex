...
\[
	\begin{aligned}
		...
		\tilde{f}_p &= -\frac{C_p}{J}\dot{\theta} + \frac{C_p}{J}\dot{\theta}_r. 	
	\end{aligned}
\]
Здесь $f_\theta(\theta,I)$ это то, что мы более-менее знаем и можем воспроизвести, а $\tilde{f}_\theta$ это то, чего мы не знаем или не измеряем. Также в последнее можно включить ошибки в оценивании параметров. 

Для маховика запишем
\[
...
\[
	\begin{aligned}
		f_r(\theta,I) &= \frac{(J+Jr)K}{JJr}I - \frac{mlg}{J}\sin(\theta), \\ 
		\tilde{f}_r &= -\frac{C_p(J+Jr)}{JJ_r}\dot{\theta} + \frac{C_p}{J}\dot{\theta}_r. 	
	\end{aligned}
\]

Видно, что мы везучие, и в $\tilde{f}$ у нас входят только трения, которыми мы будем пренебрегать. Тогда
\[
	\begin{aligned}
...
\[
	...
	\end{aligned}
\]
где $e_p = \hat{x}_{p 1} - \theta$ и $e_r = \hat{x}_{r 1}  - \theta_r$.

Теперь надо это всё промоделировать чтобы понять примерные значения для коэффициентов. На вскидку я бы сказал $k_1=9$, $k_2=6$, $\alpha=0.75$. 
\end{document}
