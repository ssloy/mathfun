...
Тогда наше уравнение $\dot x = Ax + BU$ перепишется как $K\dot y = AKy + BU$. Домножим слева на $K^{-1}$: 
$$
\dot y = K^{-1}AKy + K^{-1}BU = \Lambda y + K^{-1}BU.
$$
...
Обозначим через $b_1$ первую координату вектора $K^{-1}BU$ (напоминаю, что у нас напряжение $U$ постоянно).
...
Но нас интересует конкретный случай, когда обе скорости нулевые. Самый простой способ это посчитать - это взять луч с направляющим вектором $(1,0,0)$, преобразовать его при помощи $K^{-1}$,
пересечь с плоскостью $y_1 = \frac{|b_1|}{\lambda_1}$, а затем точку пересечения обратно преобразовать при помощи $K$.
