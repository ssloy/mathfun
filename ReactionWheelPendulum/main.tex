...
\[
	\begin{aligned}		
		\dot{\tilde{x}} &= \begin{bmatrix} 0 & 1 &  0\\
								0 & 0 & -a_1\cos(y_1-\hat{x}_3)\\
								 0 & 0 & 0
							 \end{bmatrix}\tilde{x} + f(y) = A(q)\tilde{x} + f(y), \\
		\tilde{y} &= \begin{bmatrix} 1 & 0 & 1 \end{bmatrix}\tilde{x} = C\tilde{x}.
	\end{aligned}							
\]
У нас всё та же линейная система с переменными параметрами, которая становится ненаблюдаемой при $\cos(x_1+\tilde{x}_3) = 0$. Возможным выбором будет линейный наблюдатель вида $f = -L\tilde{y}$, тогда задача сводится к устойчивости для всех $q$ системы 
\[
	\dot{\tilde{x}} = \left(A(q)-LC\right)\tilde{x}.
\]
Если мы допустим, что у нас маятник всегда в верхнем полукруге, и что мы можем предположить $a_1\cos(x_1+\tilde{x}_3)\ge\sigma_0>0$, то задача сводится к решаемым численно LMI. 

Моделирование показывает переходные процессы длительностью 0.5 секунды при выборе 
\[
	L=\begin{bmatrix}546 & 1100 & -508\end{bmatrix}^\top.
\]

\begin{itemize}
\item С помощью численных LMI нельзя (в лоб) найти решение для всех $q$, так как есть выколотая точка. Однако так как маятник в этой точке стоять не будет, то, вероятно, можно построить линейный наблюдатель, сходящийся везде, кроме этой точки, и не расходящейся в ней. Так как система третьего порядка, то это можно попробовать сделать на уровне коэффициентов характеристического полинома. Я пока не могу сообразить, можно ли это переписать в виде какого-то LMI, чтобы решать численно.
